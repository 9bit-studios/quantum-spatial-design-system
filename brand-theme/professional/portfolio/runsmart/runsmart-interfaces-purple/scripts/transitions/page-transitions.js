...
const PageTransitions = {
    ...
    if (this.overlay) {
      ...
      fetch(targetUrl)
        ...
        .then(html => {
          ...
          const targetPhoneFrame = doc.querySelector('.phone-frame');
          const targetStyles = doc.querySelectorAll('style');
          ...
          // Clone entire phone frame (includes status-bar + screen)
          const underlayPhoneFrame = targetPhoneFrame.cloneNode(true);
          underlay.appendChild(underlayPhoneFrame);
  ...
  initUnderlayScripts(underlay) {
    ...
    console.log(`Found ${settingsIcons.length} settings icons in underlay`);
    ...
    settingsIcons.forEach(icon => {
      ...
      newIcon.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('Settings icon clicked in underlay - triggering slide transition');
        const targetUrl = newIcon.getAttribute('href') || 'mobile-coach-settings.html';
        this.slideInFromRight(targetUrl);
      }, { capture: true });
    });

    // Find back buttons and attach slide-out listeners
    const backButtons = underlay.querySelectorAll('.back-button');
    backButtons.forEach(button => {
      ...
      newButton.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        const targetUrl = newButton.getAttribute('href') || 'mobile-coach-dashboard.html';
        this.slideOutToRight(targetUrl);
      }, { capture: true });
    });
