 ...
 */
export default function QuantumSpatialfoundationystem(props) {
    const { 
        variant = "quantum",
        layout = "responsive",
        showSystemUI = false,
    ...
    const [selectedComponentData, setSelectedComponentData] = React.useState(null);
    
    // Container ref for size detection
    const containerRef = React.useRef(null);
    
    // Advanced color system - Onyx + Eggplant Dual Theme
    const colorSystem = {
        onyx: {
            black: "#000000",              // Pure onyx black
            nearBlack: "#050505",          // Variant
    ...
    const typographySystem = {
        fontFamily: "'SF Pro Display', -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif",
        fontWeights: {
            regular: 400,
            medium: 500,
            semibold: 600,
    ...
    const pixelDefinitions = [
        ...
        { name: "Quantum Cube", state: "materialized", primaryColor: "#2C5F2D", accentColor: "#3DFF74", secondaryColor: "#1B3D1A" },
        { name: "Spatial Octahedron", state: "materialized", primaryColor: "#331F4A", accentColor: "#5AC8FA", secondaryColor: "#1E1F5C" },
        { name: "Void Sphere", state: "materialized", primaryColor: "#6A3093", accentColor: "#BF4080", secondaryColor: "#FF2D55" },
        { name: "Heritage Block", state: "materialized", primaryColor: "#2C5F2D", accentColor: "#3DFF74", secondaryColor: "#1B3D1A" },
        
        // Partial State (4 variants)
        { name: "Phase Shift", state: "partial", primaryColor: "#331F4A", accentColor: "#5AC8FA", secondaryColor: "#1E1F5C" },
        { name: "Quantum Ripple", state: "partial", primaryColor: "#6A3093", accentColor: "#BF4080", secondaryColor: "#FF2D55" },
        { name: "Dimensional Echo", state: "partial", primaryColor: "#126D71", accentColor: "#29B6F6", secondaryColor: "#131A36" },
        { name: "Transient Form", state: "partial", primaryColor: "#331F4A", accentColor: "#5AC8FA", secondaryColor: "#1E1F5C" },
        
        // Energy State (4 variants)
        { name: "Energy Grid", state: "energy", primaryColor: "#FF2D55", accentColor: "#BF4080", secondaryColor: "#6A3093" },
        { name: "Quantum Core", state: "energy", primaryColor: "#5AC8FA", accentColor: "#29B6F6", secondaryColor: "#126D71" },
        { name: "Void Stream", state: "energy", primaryColor: "#6A3093", accentColor: "#BF4080", secondaryColor: "#FF2D55" },
        { name: "Ethereal Circuit", state: "energy", primaryColor: "#3DFF74", accentColor: "#29B6F6", secondaryColor: "#2C5F2D" },
        
        // Superposition State (4 variants)
        { name: "Quantum Plural", state: "superposition", primaryColor: "#FFFFFF", accentColor: "#5AC8FA", secondaryColor: "#FF2D55" },
        { name: "Echo Chamber", state: "superposition", primaryColor: "#FFFFFF", accentColor: "#BF4080", secondaryColor: "#6A3093" },
        { name: "Dimensional Fold", state: "superposition", primaryColor: "#FFFFFF", accentColor: "#3DFF74", secondaryColor: "#2C5F2D" },
        { name: "Reality Cascade", state: "superposition", primaryColor: "#FFFFFF", accentColor: "#29B6F6", secondaryColor: "#126D71" }
    ];
    ...
    const getColors = () => {
        const variantColors = {
            onyx: {
                primary: colorSystem.onyx.black,
                accent: colorSystem.button.primaryDark,
                background: colorSystem.onyx.black,
                surface: "rgba(11, 1, 13, 0.6)",
                text: "rgba(255, 255, 255, 0.9)",
                border: "rgba(255, 255, 255, 0.1)"
            },
            eggplant: {
                primary: colorSystem.eggplant.deepnight,
                accent: colorSystem.accents.subtleCyan,
                background: colorSystem.eggplant.deepnight,
                surface: "rgba(11, 1, 13, 0.6)",
                text: "rgba(255, 255, 255, 0.9)",
                border: "rgba(255, 255, 255, 0.1)"
            },
        ...
        return tokens?.colors || variantColors[variant] || variantColors.eggplant;
    };
    ...
    const renderDimensionalGrid = () => {
        ...
        return (
            <div style={{
                ...
                right: 0,
                bottom: 0,
                backgroundImage: `linear-gradient(to right, ${colors.accent}${Math.floor(opacity * 255).toString(16).padStart(2, '0')} 1px, transparent 1px),
                                  linear-gradient(to bottom, ${colors.accent}${Math.floor(opacity * 255).toString(16).padStart(2, '0')} 1px, transparent 1px)`,
                backgroundSize: `${size}px ${size}px`,
                pointerEvents: "none",
                zIndex: 1,
                transform: enable3D ? "perspective(1000px) rotateX(60deg) scale(1.5)" : "none",
        ...
        return (
            <div
                ...
                style={{
                    ...
                    filter: isSelected ? `drop-shadow(0 0 ${size/4}px ${pixelDef.accentColor})` : "none",
            ...
            >
                <canvas
                    ...
                    ref={canvas => {
                        if (canvas) renderPixelToCanvas(canvas, pixelDef, enable3D);
                    }}
                />
            </div>
        );
    ...
    const renderPixelToCanvas = (canvas, pixelDef, is3D) => {
        ...
        switch(pixelDef.state) {
            case 'materialized':
                renderMaterializedPixel(ctx, centerX, centerY, size, pixelDef, is3D);
                break;
            case 'partial':
            ...
            case 'energy':
                ...
                break;
            case 'superposition':
                renderSuperpositionPixel(ctx, centerX, centerY, size, pixelDef, is3D);
                break;
        }
    };
    
    // Pixel rendering implementations
    const renderMaterializedPixel = (ctx, centerX, centerY, size, pixelDef, is3D) => {
        ctx.save();
        ...
        if (is3D) {
            ...
            ctx.beginPath();
            ctx.roundRect(-pixelSize/2, -pixelSize/2, pixelSize, pixelSize, 4);
            ctx.fill();
            ctx.stroke();
        }
    ...
    const renderPartialPixel = (ctx, centerX, centerY, size, pixelDef, is3D) => {
        ctx.save();
        ctx.translate(centerX, centerY);
    ...
    const renderSuperpositionPixel = (ctx, centerX, centerY, size, pixelDef, is3D) => {
        ctx.save();
        ctx.translate(centerX, centerY);
        ...
        return (
            <div ref={containerRef} style={{
                ...
                <div style={{
                    ...
                    {deviceInfo?.isM4Detected && <div style={{ fontSize: 12, marginTop: 4 }}>M4 optimizations enabled</div>}
                </div>
            </div>
        );
    }
    ...
    const filteredPixels = pixelState === 'all' ? 
        pixelDefinitions : 
    ...
    return (
        <div ref={containerRef} style={containerStyle}>
                ...
                {showTypography && (
                    <div style={{ marginBottom: spacing.xl }}>
                        <h2 style={{
                            ...
                            fontWeight: typographySystem.fontWeights.semibold,
                            color: colors.accent
                        }}>
                            Typography System
                        </h2>
                        ...
                        <div style={{
                            ...
                            <div style={{ marginBottom: spacing.md }}>
                                <div style={{ fontSize: typographySystem.sizes.display, fontWeight: typographySystem.fontWeights.bold, lineHeight: typographySystem.lineHeights.tight }}>Display</div>
                                <div style={{ fontSize: typographySystem.sizes.xxl, fontWeight: typographySystem.fontWeights.bold }}>Heading XXL</div>
                                <div style={{ fontSize: typographySystem.sizes.xl, fontWeight: typographySystem.fontWeights.semibold }}>Heading XL</div>
                                <div style={{ fontSize: typographySystem.sizes.lg, fontWeight: typographySystem.fontWeights.medium }}>Heading Large</div>
                                <div style={{ fontSize: typographySystem.sizes.md, fontWeight: typographySystem.fontWeights.regular }}>Body Medium</div>
                                <div style={{ fontSize: typographySystem.sizes.sm, fontWeight: typographySystem.fontWeights.regular, opacity: 0.8 }}>Body Small</div>
                            </div>
                        </div>
                    </div>
                )}
                            ...
                            {filteredPixels.map(pixel => (
                                <div 
                                    ...
                                    style={{
                                        ...
                                        padding: spacing.md,
                                        border: selectedComponent === pixel.name 
                                ...
                                >
                                    {renderQuantumPixel(pixel, isCompact ? 64 : 80)}
                                    <div style={{
                        ...
                        {selectedComponentData && (
                            <div style={{
                                ...
                                <div style={{ display: "flex", gap: spacing.lg, alignItems: "center" }}>
                                    {renderQuantumPixel(selectedComponentData, 120)}
                                    <div style={{ flex: 1 }}>
                                        <div style={{ marginBottom: spacing.sm }}>
                                            <strong>State:</strong> {selectedComponentData.state.charAt(0).toUpperCase() + selectedComponentData.state.slice(1)}
                                        </div>
                                        <div style={{ marginBottom: spacing.sm }}>
                                            <strong>Primary:</strong> <span style={{ color: selectedComponentData.primaryColor }}>{selectedComponentData.primaryColor}</span>
                                        </div>
                                        <div style={{ marginBottom: spacing.sm }}>
                                            <strong>Accent:</strong> <span style={{ color: selectedComponentData.accentColor }}>{selectedComponentData.accentColor}</span>
                                        </div>
                                        <div>
                                            <strong>Secondary:</strong> <span style={{ color: selectedComponentData.secondaryColor }}>{selectedComponentData.secondaryColor}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    <p style={{
                                        ...
                                        Apple HIG compliant card with quantum-spatial styling.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div style={{
                ...
                {deviceInfo?.isM4Detected ? "M4 Enhanced" : "Standard"} • {containerSize.width}×{containerSize.height}
            </div>
        </div>
    );
}
...
addPropertyControls(QuantumSpatialfoundationystem, {
    variant: {
        title: "Variant",
        type: ControlType.Enum,
        options: ["onyx", "eggplant", "heritage", "transitional", "quantum", "superposition"],
