...
FRAMER_REFRESH_TOKEN= environment variable.');
  ...
  try {
    ...
    const teamResponse = await fetch(`${config.apiBaseUrl}/teams/${config.teamId}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${config.apiKey}`,
        'Content-Type': 'application/json'
    ...
    if (config.projectId && config.projectId !== 'your-framer-project-id') {
      ...
      const projectResponse = await fetch(`${config.apiBaseUrl}/teams/${config.teamId}/projects/${config.projectId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${config.apiKey}`,
          'Content-Type': 'application/json'
      ...
      const projectsResponse = await fetch(`${config.apiBaseUrl}/teams/${config.teamId}/projects`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${config.apiKey}`,
          'Content-Type': 'application/json'
    ...
    const librariesResponse = await fetch(`${config.apiBaseUrl}/teams/${config.teamId}/component-libraries`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${config.apiKey}`,
        'Content-Type': 'application/json'
    ...
    if (config.cloudflareApiBaseUrl) {
      console.log(`🔍 Testing connection to Cloudflare API at ${config.cloudflareApiBaseUrl}...`);
      try {
        const cloudflareResponse = await fetch(`${config.cloudflareApiBaseUrl}/design-system/tokens?state=transitional`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
